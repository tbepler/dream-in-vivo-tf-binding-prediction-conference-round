...
def model_constructor(dtype='float32'):
    ## use less units but boost - try boosting 8 times
    from src.models.resid_conv_logit import ResidConvLogit
    predictor = ResidConvLogit(units=16, layers=[3, 6, 6, 3], pool='max')
    ...
    model = BoostedBindingModel(predictor, optimizer=optimizer, dtype=dtype, num_models=8)
    return model
