...
def model_constructor(dtype='float32'):
    ...
    predictor = ResidConvLogit(units=4, layers=[3, 6, 6, 3], pool='max')
    from src.models.binding_model import BindingModel, Regularizer
    from src.models.optimizer import RMSprop, Nesterov, SGD
    optimizer = SGD(nu=0.001, conditioner=RMSprop(), momentum=Nesterov(), minibatch_size=64, convergence_sample=500, max_iters=32)
