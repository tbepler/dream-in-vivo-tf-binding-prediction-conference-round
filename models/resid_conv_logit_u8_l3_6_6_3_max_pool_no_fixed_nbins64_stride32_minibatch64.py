...
def load_model(dtype='float32'):
    ...
    predictor = ResidConvLogit(units=8, layers=[3, 6, 6, 3], pool='max', use_fixed=False)
    from src.models.binding_model import BindingModel, Regularizer
    from src.models.optimizer import RMSprop, Nesterov, SGD
    optimizer = SGD(nu=0.001, conditioner=RMSprop(), momentum=Nesterov(), minibatch_size=64, chunk_size=64*8, convergence_sample=500)
