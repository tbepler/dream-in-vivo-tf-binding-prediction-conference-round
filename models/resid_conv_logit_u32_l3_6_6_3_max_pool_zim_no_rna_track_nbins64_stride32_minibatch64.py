...
def load_data(TF, config={}):
    ...
    if X.fixed is not None:
        ## only keep cell lines relevant to this TF in the fixed data
        from src.dataset import get_training_cell_lines
        cell_lines = get_training_cell_lines(TF)
        X.fixed.df = X.fixed.df[cell_lines]
    return (X, Y)

def load_model(dtype='float32'):
    from src.models.resid_conv_logit import ResidConvLogit
    predictor = ResidConvLogit(units=32, layers=[3, 6, 6, 3], pool='max', use_bias=True
                              , fixed_l1=0, fixed_l2=0)
    from src.models.preprocess import ZeroInflatedMean
    preprocess = ZeroInflatedMean(use_rna_seq_track=True) #use_rna_seq_track=False)
    ...
    regularizer = Regularizer(l2=1e-7) #, l1=1e-7)
